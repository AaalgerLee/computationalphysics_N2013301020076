...
X,Y= np.meshgrid(x,x)
ax.plot_surface(X,Y,V_whole, rstride=1, cstride=1, cmap='hot')
ax.set_xlabel('x')
ax.set_ylabel('y')
...
ax.set_ylim(-1,1)
ax.set_zlim(-1.2,1.2)
ax.contourf(X, Y, V_whole, zdir='z', offset=-1.2, cmap=plt.cm.hot)
plt.show()

#2D contour
plt.contourf(X, Y, V_whole, 10, alpha=.75, cmap='hot')
